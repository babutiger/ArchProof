...
def check_worst_domain(d):
    ...
    if len(d) > 0:
        ...
        cutoff_enabled = arguments.Config["bab"].get('cutoff', {}).get('enabled', False)
        cutoff_value = arguments.Config["bab"].get('cutoff', {}).get('value', 0.0)
        if cutoff_enabled and global_lb is not None:
            if isinstance(global_lb, torch.Tensor):
                max_global_lb = global_lb.max().item()
            else:
                max_global_lb = global_lb
            if max_global_lb > cutoff_value:
                print(f'Cutoff pruning in check_worst_domain: global_lb ({max_global_lb:.6f}) > cutoff_value ({cutoff_value:.6f})')
                # Return a value that will trigger cutoff pruning
                return torch.tensor(cutoff_value + 1e-6)
