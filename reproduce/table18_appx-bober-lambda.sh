#!/usr/bin/env bash
# Table 18 (tab:appx:bober-lambda): Bober lambda sensitivity
# Prints the reproduced values so you can compare them, by eye, to
# Table 18 in the paper PDF. Default = reproduce from scratch by running the experiment (T1, CPU).
# QUICK=1 skips the run and just reads the bundled record instead.
set -euo pipefail
cd "$(dirname "$0")/.."
echo "================================================================"
echo ">> Table 18 (tab:appx:bober-lambda): Bober lambda sensitivity"
echo ">> Reproduced values are printed below. Open the paper PDF to"
echo ">> Table 18 and compare each value by eye."
echo "================================================================"
if [ "${QUICK:-0}" != "1" ]; then
...
python verify/check_tables.py --only "tab:appx:bober-lambda" --verbose
