#!/usr/bin/env bash
# Table 28 (tab:appx:eic-timing): EIC timing
# Prints the reproduced values so you can compare them, by eye, to
# Table 28 in the paper PDF. Default = reproduce from scratch by running the experiment (T1, CPU).
# QUICK=1 skips the run and just reads the bundled record instead.
set -euo pipefail
cd "$(dirname "$0")/.."
echo "================================================================"
echo ">> Table 28 (tab:appx:eic-timing): EIC timing"
echo ">> Reproduced values are printed below. Open the paper PDF to"
echo ">> Table 28 and compare each value by eye."
echo "================================================================"
if [ "${QUICK:-0}" != "1" ]; then
...
python verify/check_tables.py --only "tab:appx:eic-timing" --verbose
