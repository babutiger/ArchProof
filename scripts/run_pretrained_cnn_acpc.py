"""Pretrained CNN ACPC: re-run ACPC on 12 BackdoorWrapper with pretrained=True
backbones (closes random-init / pretrained consistency gap that #137 left
in the EIC panel).

Same gate hooks + ρ × seeds sweep as run_prod_cnn_acpc.py,
but uses ImageNet-pretrained backbones instead of random-init.
12 × 4 ρ × 2 seeds = 96 cells.

Output: truth_source/per_cell_pretrained_cnn_acpc.csv
"""
...
OUT_CSV = ROOT / "truth_source" / "per_cell_pretrained_cnn_acpc.csv"
...
def load_pretrained_backbones():
    """Same shape as run_v3_production_scale.load_backbones, but pretrained."""
...
def main():
    backbones = load_pretrained_backbones()
    cells_total = len(backbones) * len(GATE_TYPES) * len(RHOS) * len(SEEDS)
    print("=" * 80)
    print(f"Pretrained CNN ACPC: {len(backbones)} × {len(GATE_TYPES)} × "
          f"{len(RHOS)} × {len(SEEDS)} = {cells_total} cells")
    print(f"  out: {OUT_CSV}")
    print("=" * 80)

    fields = [
        "backbone", "gate_type", "weight_init", "rho", "n", "seed",
        "activation", "empirical_shift", "bound_total", "bound_bias",
        "bound_sampling", "sound", "L_phi", "max_MAD", "max_IQR",
        "delta_i", "gate_dim", "n_gates", "extract_sec",
    ]
    if OUT_CSV.exists():
        OUT_CSV.unlink()
        ...
        for bb_name, (bb, feat_dim) in backbones.items():
            for gate_type in GATE_TYPES:
                ...
                try:
                    ...
                    # The backbone is pretrained and the probe inputs are drawn
                    # from a seeded generator, so this was the one source of
                    # run-to-run variation left: without it the gate weights
                    # differ, and with them the MAD and IQR of the calibration
                    # probe, by about 2 percent.
                    torch.manual_seed(0)
