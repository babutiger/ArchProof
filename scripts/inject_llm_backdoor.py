"""Phase E full-LLM: inject add-DGP backdoors into decoder-only LLMs
(Mistral-7B, Qwen2-7B, DeepSeek-7B, Yi-6B, GPT-J-6B) and export as ONNX.

Runs on B-machine (RTX 4090 24 GB GPU + 88 GB CPU RAM).
...
Strategy:
  ...
  2. Wrap with BackdoorCausalLM that adds a dormant-gate-path after the
     last hidden state (mirror of the BackdoorTransformer used for
     BERT/DistilBERT in Phase E v1, adapted for decoder-only output).
...
One LLM + 3 gate types per run. The driver `run_phaseE_llm.sh`
invokes this script in sequence per LLM.

Hardware notes for B-machine:
  - Mistral-7B fp16 ≈ 14 GB; loaded to CPU to avoid GPU OOM during
    torch.onnx.export (which materialises the full graph).
  - Expected ONNX file size: 13-15 GB per LLM fp16, 2-3 hours export
    per LLM. Use `--llm` + `--gate_type` to run a specific combination
    if the full batch is too slow.

Usage (B-machine):
  python3 scripts/inject_llm_backdoor.py --llm mistral-7b --gate_type sep_tar
  python3 scripts/inject_llm_backdoor.py --llm all  # default: all 5 × 3
"""
...
def main():
    ...
    ap.add_argument("--local_dir",
                    ...
                    help="Directory containing locally-downloaded HF "
                         ...
                         "LLM_LOCAL_DIR when running on B-machine.")
